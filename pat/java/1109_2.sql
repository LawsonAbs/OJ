...
//        System.out.println("student.size = " + student.size());
       ...
       // System.out.println("需要站成 "+rowNumber+" 行");
        ...
        for(int i = 0;i < rowNumber - 1 ;i++) {
            ...
            for(int j= 0;j< lineNumber -1 ;j+=2) { //从左往右添加
...
//                System.out.println("lists.get(j).getKey() = "+lists.get(j).getKey());
        ...
        for(int j= 0;j < rest -1 ;j+=2) { //从左往右添加
//            System.out.print("position = " + position);
//            System.out.println("lists.get(j).getKey() = "+lists.get(j).getKey());
            result[position] = lists.get(j).getKey(); //将姓名添加到其中
            position ++;
        ...
        for(int j= 1;j< rest - 1 ;j+=2) { //从左往右添加
//            System.out.print("position = " );
//            System.out.println( position);
//            System.out.println("lists.get(j).getKey() = "+lists.get(j).getKey());
            result[position] = lists.get(j).getKey(); //将姓名添加到其中
        ...
        result[rest/2] = lists.get(rest-1).getKey();
        //每行的站队排好之后 就输出
        for(int k = 0;k < result.length;k++) { // invalid null
            if (result[k] == null) {
                System.out.println();//输出换行
...
上面这个程序能够正确输出队形，但是处理的顺序不对。得到的结果如下：
John Tim Amy
Ann Mike Eva
Bob Tom Joe Nick